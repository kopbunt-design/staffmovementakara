...
// ทำครบทุกวันของเดือน -> เต็มอัตราเสมอ ไม่ว่าเดือนนั้นจะมีกี่วัน (ผู้ใช้ยืนยัน 2026-08-18)
const feb = n => month("E1",2026,2, Array.from({length:n},(_,i)=>["D01","A01","N01"][i%3]));
eq(one(feb(28)).total, 1800, "ก.พ. 28 วันทำครบ -> เต็ม 1,800 เท่ากับเดือนอื่นที่ทำครบ");
eq(one(feb(28)).wholeMonth, true, "ติดธงว่าทำครบทั้งเดือน");
{
  // ก.พ. ทำไม่ครบ -> กลับไปคิด 60 x วันที่จ่าย
  const rows = feb(28); rows[0] = { ...rows[0], Deduct_Day:1 };
  eq([one(rows).payDays, one(rows).total, one(rows).wholeMonth], [27, 1740, false],
     "ก.พ. ขาด 1 วัน -> 60 x 27 = 1,620");
}
eq(one(feb(24)).total, 1440, "ไฟล์มีแค่ 24 วัน (ไม่ครบเดือน) -> 60 x 24 = 1,440");
eq(one(month("E1",2026,7, jul31(["D01","A01","N01"]))).wholeMonth, true, "ก.ค. 31 วันทำครบ -> เต็ม");
eq(one(month("E1",2026,7, jul31(["D01","A01","N01"]))).capped, true, "ก.ค. 31 วัน -> ติดเพดาน");
eq(one(feb(28)).capped, false, "ก.พ. 28 วัน -> เต็มแต่ไม่ได้ติดเพดาน");
{
  // เดือน 30 วัน ทำครบ -> เต็มเช่นกัน (ทั้งสองวิธีให้ผลเท่ากันพอดี)
  const jun = month("E1",2026,6, Array.from({length:30},(_,i)=>["D01","A01","N01"][i%3]));
  eq([one(jun).total, one(jun).wholeMonth], [1800, true], "มิ.ย. 30 วันทำครบ -> 1,800");
}
{
  ...
  eq(one(d19).wholeMonth, false,
     "เข้าใหม่กลางเดือนแล้วทำครบทุกวันที่จ้าง ต้องไม่นับเป็น 'ครบทั้งเดือน' (ไม่งั้นจะได้ 1,800 ผิดจากเฉลย)");
...
eq([a1.payDays, a1.total], [30, 1740], "ขาด 1 วัน -> 1,800 - 60 = 1,740 (เดือน 31 วันก็หัก)");
...
eq([one(absent2).payDays, one(absent2).total], [29, 1680], "ขาด 2 วัน -> 1,800 - 120 = 1,680");

// ---------- หักวันขาดด้วยฐาน 30 คงที่ (ผู้ใช้เลือก 2026-09-21) ----------
// เดิมคิดเป็น payDays x อัตรา/30 ตรง ๆ ทำให้เดือน 31 วันมีวันขาดวันแรกฟรี
// ตอนนี้หายอด "ถ้าไม่ขาดงาน" ก่อน แล้วหักวันขาดออก -> ขาด 1 วันหัก 60 เสมอทุกเดือน
{
  const absN = n => {
    const rows = month("E1",2026,7, jul31(["D01","A01","N01"]));
    for (let i=0;i<n;i++) rows[i] = { ...rows[i], Deduct_Day:1 };
    return one(rows);
  };
  eq(absN(0).total, 1800, "ก.ค. ไม่ขาด -> 1,800");
  eq(absN(1).total, 1740, "ขาด 1 -> หัก 60");
  eq(absN(3).total, 1620, "ขาด 3 -> หัก 180");
  // ขาดเยอะจนเหลือ 2 วัน = ได้เข้าแค่ 2 ตระกูลกะ อัตราเดือนตกจาก 1,800 เป็น 1,200
  // ฐานหักจึงเป็น 1,200/30 = 40 ไม่ใช่ 60 — กติกาจำนวนตระกูลกะมาก่อนการหักวันขาดเสมอ
  eq([absN(29).monthlyRate, absN(29).total], [1200, 40], "ขาด 29 -> 1,200 - 29x40 = 40");
  eq(absN(30).total, 0,   "ขาด 30 -> เหลือกะเดียว อัตราเป็น 0 อยู่แล้ว");
  eq(absN(31).total, 0,   "ขาดทั้งเดือน -> 0 ไม่ติดลบ");
  eq(absN(2).absentDays, 2, "รายงานจำนวนวันขาดออกมาด้วย");
  // เดือนสั้นกับเดือนยาวต้องหักเท่ากัน — นี่คือประเด็นทั้งหมดของการเปลี่ยนกติกา
  const febAbs1 = (() => {
    const rows = month("E1",2026,2, Array.from({length:28},(_,i)=>["D01","A01","N01"][i%3]));
    rows[0] = { ...rows[0], Deduct_Day:1 };
    return one(rows);
  })();
  eq(febAbs1.total, 1740, "ก.พ. ขาด 1 -> 1,740 เท่ากับ ก.ค. ขาด 1");
}

// ---------- สิ่งที่ต้องไม่เปลี่ยน — เฉลยจริง ก.ค. 2026 ----------
{
  // คนเข้าใหม่กลางเดือน: ถูกจ้าง 19 วัน ไม่ได้ขาดงาน -> ยังต้องได้ 1,140 ตามเฉลย
  const d19 = month("E1",2026,7, jul31(["D01","A01","N01"])).slice(0,19);
  eq([one(d19).absentDays, one(d19).total], [0, 1140], "จ้าง 19 วัน ไม่ขาด -> 1,140 (เฉลยจริง)");
  // ถูกจ้าง 30 วัน ไม่ได้ขาดงาน -> ยังต้องได้ 1,800 ตามเฉลย
  // ต่างจาก "จ้างครบ 31 วันแต่ขาด 1 วัน" ซึ่งได้ 1,740 — สองเคสนี้ต้องแยกกันให้ได้
  const d30 = month("E1",2026,7, jul31(["D01","A01","N01"])).slice(0,30);
  eq([one(d30).absentDays, one(d30).total], [0, 1800], "จ้าง 30 วัน ไม่ขาด -> 1,800 (เฉลยจริง)");
}

// ---------- วันที่ไม่จ่ายเพราะกติกากะ ต้องไม่ถูกนับเป็นวันขาด ----------
{
  // NOR สลับ N03 (<= 15 วัน) จ่ายเฉพาะวัน N03 — วัน NOR ไม่ใช่การขาดงาน ห้ามหักซ้ำ
  // 31 วัน เข้า N03 ทุก ๆ วันที่ 3 = 11 วัน (ไม่เกินเกณฑ์ 15 วัน)
  const rows = month("E1",2026,7, jul31(["N03","NOR","NOR"]));
  const r = one(rows);
  eq([r.absentDays, r.payDays], [0, 11], "วัน NOR ไม่ใช่วันขาดงาน");
  eq(r.total, 440, "11 วัน x (1,200/30) = 440 ตามเดิม ไม่ถูกหักเพิ่ม");
}
...
// พักงาน = ไม่จ่ายวันนั้น แม้จะมีเวลาเข้างานติดมาด้วย
{
  ...
  eq(r.total, 1620, "พักงาน 3 วัน -> 1,800 - 180 = 1,620");
}
...
const sickDay = r => ({ ...r, Check_In:"", Leave_No_Deduct:1, Leave_Type:"Sick Leave" });
{
  ...
  eq([res.summary[0].payDays, res.summary[0].total], [27, 1560], "ลาป่วยยาว 4 วัน -> 1,800 - 240 = 1,560");
}
{
...
}
{
  ...
  eq(res.summary[0].total, 1500, "ลาป่วยไม่จ่ายเงิน 5 วัน -> 1,800 - 300 = 1,500");
