...
import { allEmployees, allMovements, movYM, sepYM, hcAtMonthEnd, esc, toast } from "./app.js";
...
function buildData(periodYMs) {
  ...
  for(const ym of periodYMs){
    ...
    const movVolC=new Set(allMovements.filter(v=>sepYM(v.date)===ym&&["Resignation","Retirement"].includes(v.type)).map(v=>v.emp_code));
    const empVol=allEmployees.filter(e=>sepYM(e.end_date)===ym&&["Resigned","Retired"].includes(e.status)&&!movVolC.has(e.emp_code));
    ...
    const movInvC=new Set(allMovements.filter(v=>sepYM(v.date)===ym&&v.type==="Termination").map(v=>v.emp_code));
    const empInv=allEmployees.filter(e=>sepYM(e.end_date)===ym&&e.status==="Terminated"&&!movInvC.has(e.emp_code));
